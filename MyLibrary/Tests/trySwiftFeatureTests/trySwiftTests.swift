...
final class trySwiftTests: XCTestCase {
  ...
  @MainActor
  func test_view_websiteTapped_onIOSAndMacOS_presentSafari() async throws {
    #if os(visionOS)
      throw XCTSkip("this test is for iOS and macOS")
    #endif
    ...
    await store.send(.view(.websiteTapped)) {
      $0.destination = .safari(.init(url: URL(string: "https://tryswift.jp")!))
    }
  }

  @MainActor
  func test_view_websiteTapped_onVisionOS_openURL() async throws {
    #if os(iOS) || os(macOS)
      throw XCTSkip("this test is for visionOS")
    #endif
